...
public class SysMenuServiceImpl implements ISysMenuService {
    ...
    @Override
    @RedisCache(type = ZtreeVo.class)
    public List<ZtreeVo> findAllListByZtree(HashMap<String, Object> params){
        Long roleId = null;
        if (params.get("roleId") == null) {
            SysUserRole sysUserRole = sysUserRoleService.findSysUserRoleByUserId(ShiroUtils.getUserId());
            roleId = sysUserRole.getRoleId();
        ...
        params.put("parentId",0);
        List<ZtreeVo> listByZtree = sysMenuMapper.findListByZtree(params);
        List<ZtreeVo> childZtreeList = null;
        List<ZtreeVo> childZtreeList2 = null;
        for(ZtreeVo ztreeVo : listByZtree){
            log.debug("[ 获取{}  下的二级菜单] -------------->"+ztreeVo.getName());
            params.put("parentId",ztreeVo.getId());
            childZtreeList = sysMenuMapper.findListByZtree(params);
            for (ZtreeVo childZtreevo : childZtreeList) {
                log.debug("[ 获取{}  下的三级菜单] -------------->"+childZtreevo.getName());
                params.put("parentId",childZtreevo.getId());
                childZtreeList2 = sysMenuMapper.findListByZtree(params);
                childZtreevo.setChildren(childZtreeList2);
            }
            ztreeVo.setChildren(childZtreeList);
        }
        return listByZtree;
    }
