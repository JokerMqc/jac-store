...
layui.use(['laydate', 'laypage', 'layer', 'table', 'form', 'element'], function () {
    ...
    var saveSysRole = function (data) {
        ...
        if (data.id != null && data.id != '') {
            url = serverPath + '/sys/role/updateSysRole.do';
        } else {
            url = serverPath + '/sys/role/saveSysRole.do';
        }
    ...
    var delSysRole = function (roleId) {
        $.post(serverPath + '/sys/role/delSysRoleById.do', {
            id: roleId
        }, function (data) {
            if (data.code == '0') {
                layer.closeAll();
    ...
    var updateRoleMenu = function (addMenuList, removeMenuList) {
        $.ajax({
            type: "POST",
            url: serverPath + "/sys/role/menu/updateRoleMenuByList.do",
            dataType: "json",
            traditional: true,
            data: {
     ...
     *  获取所有的菜单树形
     */
    var getAllZtreeList =function (roleId) {
        $.ajax({
            type: "POST",
            url: serverPath + '/sys/role/menu/findAllMenuByZtree.do?roleId='+roleId,
            dataType: "json",
            async: true,
            success: function (data) {
                // 数据回显
    ...
    var setting = {
        ...
        //         async: {
        //             enable: true,
        //             url: serverPath + '/sys/menu/findAllListByZtree.do?roleId='+roleId,
        //             autoParam: ["id"],
