...
layui.define(['jquery', 'element', 'nprogress', 'utils'], function(exports) {
    ...
    var _tab = {
        ...
        createTabDom: function() {
            ...
            //模板
            var tpl = [
                    // '<div class="chansy_side_icon">',
                    // '<div class="admin-side-toggle kit-side-fold"><i class="iconfont">&#xe6f5;</i></div>',
                    // '<div class="admin-side-full" onClick="addd();"><i class="iconfont" >&#xe682;</i></div>',
                    // '</div>',
        ...
        tabDelete: function(layId) {
            ...
            //面条兄修复的关闭最后一个tab页面空白的bug
            var _liList=this._title[0];
            var _span=$(_liList).find("span");
            if(_span[0]!=undefined){
                $(_span[0]).removeClass("layui-this");
            }
            var _thisLi=$(_liList).find(".layui-this") ;

            if(_thisLi[0]!=undefined){

            }else{
                //获取最后一个节点添加
                var _lastLi=$(_liList).children("li").last();
                $(_lastLi).toggleClass("layui-this").click();
            }
        ...
        tabAdd: function(options) {
            ...
            var span=that._title[0];
            var cc=$(span).find("span");
            console.log("cc[0]",cc[0]);
            if(cc[0]!=undefined){
                $(cc[0]).before(titleHtm.join(''));
            }else{
