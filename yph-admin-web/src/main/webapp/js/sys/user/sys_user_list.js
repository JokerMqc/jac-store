...
layui.use(['laydate', 'laypage', 'layer', 'table', 'form', 'element'], function () {
    ...
    var getTableList = function () {
        $.post(serverPath + "/sys/user/findSysUserListByPage.do", {
    ...
    var saveSysUser = function (data) {
        ...
        if (data.id != null && data.id != '') {
            url = serverPath + '/sys/role/updateSysUser.do';
        } else {
            url = serverPath + '/sys/role/saveSysUser.do';
        }
    ...
    var delSysUser = function (roleId) {
        $.post(serverPath + '/sys/role/delSysUserById.do', {
            id: roleId
        }, function (data) {
            if (data.code == '0') {
                layer.closeAll();
    ...
    var batchDelSysUserByIds = function (ids) {
        $.ajax({
            type: "POST",
            url: serverPath + "/sys/menu/batchDelSysUserByIds.do",
            dataType: "json",
            traditional: true,
            data: {
                ids: ids
